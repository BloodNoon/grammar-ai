...
export default function PromptManager({ id, prompt, onClose }) {
	...
	useEffect(() => {
		setValue('structure', structureInput);
	}, [structureInput]);
				...
				<form onSubmit={handleSubmit(prompt ? editPrompt : createPrompt)}>
					<ModalBody>
						...
						<FormControl
							...
							alignItems="center"
						>
							<FormLabel htmlFor="haveStructure">
								Sentence Structure
							</FormLabel>
							<Controller
								render={() => (
						...
						{structureState && (
							<FormControl>
								<Controller
									...
									render={() => (
										<Input
											...
											onChange={(e) =>
												setStructureInput(() => e.target.value)
											}
								...
								<Text as="i" fontSize="xs" color="gray.400">
									Note: Part of speech only recognizable by the system
									if '#' exists. For you own words, type them above
									manually without the '#'. The buttons below only adds
									part of speech to the end, so do move them to the
									right position with each separated with a space.
								</Text>
