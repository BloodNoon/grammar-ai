...
const PossessiveSorter = () => {
  ...
  const renderWord = (item) => (
    <div
      ...
      style={{
        padding: '6px 12px', background: '#fff', border: '2px solid #cbd5e1', borderRadius: '6px',
  ...
  return (
    <div style={{ width: '100%' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '15px' }}>
        <div onDrop={(e) => handleDrop(e, 'singular')} onDragOver={handleDragOver} style={{ minHeight: '100px', padding: '10px', background: '#e0f2fe', border: '2px dashed #0284c7', borderRadius: '8px' }}>
          <strong style={{ color: '#0369a1', display: 'block', textAlign: 'center', marginBottom: '8px' }}>Singular ('s)</strong>
          {singular.map(renderWord)}
        </div>
        <div onDrop={(e) => handleDrop(e, 'regularPlural')} onDragOver={handleDragOver} style={{ minHeight: '100px', padding: '10px', background: '#ffedd5', border: '2px dashed #ea580c', borderRadius: '8px' }}>
          <strong style={{ color: '#c2410c', display: 'block', textAlign: 'center', marginBottom: '8px' }}>Plural (s')</strong>
          {regularPlural.map(renderWord)}
        </div>
        <div onDrop={(e) => handleDrop(e, 'irregularPlural')} onDragOver={handleDragOver} style={{ minHeight: '100px', padding: '10px', background: '#f3e8ff', border: '2px dashed #9333ea', borderRadius: '8px' }}>
