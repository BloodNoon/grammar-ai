...
const getTokenStyle = (token, state) => {
  ...
  const base = {
    ...
    borderRadius: "8px",
    fontSize: "1.1rem",
    fontWeight: token.isAdv ? "700" : "500",
  ...
  if (token.isAdv)             return { ...base, background: "#fef9c3", color: "#854d0e", border: "1.5px solid #fde047" };
  return { ...base, color: "#374151" };
};

const AdverbHuntParagraph = ({ tokens, tokenStates, onTokenClick }) => (
...
export default AdverbHuntParagraph;
