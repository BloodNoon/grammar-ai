...
import PARAGRAPHS from "./adverbHuntData";
import AdverbHuntParagraph from "./AdverbHuntParagraph";
import AdverbHuntPopup from "./AdverbHuntPopup";
import AdverbHuntResults from "./AdverbHuntResults";

const AdverbHuntGame = () => {
  ...
  const totalAdv = paragraph.tokens.filter((t) => t.isAdv).length;
  ...
  const handleAnswer = (choice) => {
    ...
    setPopup(null);
    const answered = Object.keys(newStates).filter((k) => paragraph.tokens[k]?.isAdv).length;
    if (answered === totalAdv) {
      const missedList = paragraph.tokens.filter((t, i) => t.isAdv && newStates[i] !== "correct").map((t) => ({ word: t.word, type: t.type }));
  ...
  if (gameOver) {
    return (
      <div style={{ fontFamily: "'Inter', sans-serif" }}>
        <AdverbHuntResults score={score} total={totalAdv} missed={missed} onReplay={handleReplay} />
      </div>
    );
  }
    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      <div style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: "14px", padding: "16px 20px", marginBottom: "16px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
          <span style={{ fontWeight: "700", fontSize: "0.9rem", color: "#1f2937" }}>Adverbs Found</span>
          <span style={{ fontWeight: "700", fontSize: "0.9rem", color: "#1f2937" }}>{correctCount} / {totalAdv}</span>
        </div>
        <div style={{ background: "#e5e7eb", borderRadius: "999px", height: "8px" }}>
          <div style={{ background: "#22d3ee", height: "100%", borderRadius: "999px", width: `${(correctCount / totalAdv) * 100}%`, transition: "width 0.3s ease" }} />
        </div>
      </div>
      <div style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: "16px", padding: "24px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
        <div style={{ background: "#ecfeff", border: "1px solid #a5f3fc", borderRadius: "10px", padding: "10px 14px", marginBottom: "20px" }}>
          <p style={{ margin: 0, fontSize: "0.85rem", fontWeight: "600", color: "#0e7490" }}>
            📖 Click every adverb — then identify its type. Yellow words are adverbs!
          </p>
        </div>
        <AdverbHuntParagraph tokens={paragraph.tokens} tokenStates={tokenStates} onTokenClick={handleTokenClick} />
        {wrongClicks > 0 && (
      ...
      {popup && (
        <AdverbHuntPopup token={paragraph.tokens[popup.tokenIndex]} onAnswer={handleAnswer} onClose={() => setPopup(null)} position={popup.position} />
      )}
    </div>
  );
};

export default AdverbHuntGame;
