...
const AdjectivePractice = () => {
  const sortingQuestions = adjectiveData.filter((q) => q.practice_type === "sorting");
  const typingQuestions = adjectiveData.filter((q) => q.practice_type === "typing");
  const mcQuestions = adjectiveData.filter((q) => q.practice_type === "multiple_choice");

  const games = [
    { id: "mc", title: "📝 Adjective Identification Challenge", color: "green.500", component: <MultipleChoiceWrapper questionsToPlay={mcQuestions} nextPath="/adjective-structure" /> },
    { id: "typing", title: "✍️ Typing Challenge", color: "purple.500", component: <TypingGameWrapper questionsToPlay={typingQuestions} nextPath="/adjective-structure" /> },
    { id: "sorting", title: "🔀 Adjective Sorting", color: "#FF5722", component: <SortingGameWrapper questionsToPlay={sortingQuestions} nextPath="/adjective-structure" /> },
      ...
      <Box maxW="800px" mx="auto">
        {completed ? (
          <GameCard variant="game" textAlign="center" py={12}>
            ...
            <Heading size="lg" color="ink.700" mb={3}>You've completed all the Adjective games!</Heading>
            <Text fontSize="md" color="gray.600" mb={6}>Great work mastering adjectives through Multiple Choice, Typing, Sorting, and AI Practice.</Text>
          ...
          <GameCard variant="game">
            ...
            {isAI ? (
              <GrammarAIPractice topic="adjectives" onComplete={handleNext} />
            ) : (
              <>
                {current.component}
