...
const getTokenStyle = (token, state) => {
  ...
  const base = {
    ...
    borderRadius: "8px",
    fontSize: "1.1rem",
    fontWeight: token.isAdj ? "700" : "500",
  ...
  if (token.isAdj)             return { ...base, background: "#fef9c3", color: "#854d0e", border: "1.5px solid #fde047" };
  return { ...base, color: "#374151" };
};

const AdjectiveHuntParagraph = ({ tokens, tokenStates, onTokenClick }) => (
...
export default AdjectiveHuntParagraph;
