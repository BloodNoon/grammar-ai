...
const AdjectiveHuntResults = ({ score, total, missed, onReplay }) => {
  const pct = Math.round((score / total) * 100);
  const emoji = pct === 100 ? "🏆" : pct >= 70 ? "🎉" : pct >= 40 ? "👍" : "📖";
  const msg =
    pct === 100 ? "Perfect! You found every adjective!" :
    pct >= 70   ? "Great work! Almost there." :
    pct >= 40   ? "Good effort — keep practicing!" :
                  "Keep reviewing adjective types and try again!";
    ...
    <div style={{ textAlign: "center", padding: "32px 20px", maxWidth: "480px", margin: "0 auto" }}>
      ...
      {missed.length > 0 && (
        <div style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: "14px", padding: "16px 20px", marginBottom: "24px", textAlign: "left", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
          <p style={{ margin: "0 0 10px 0", fontWeight: "700", color: "#ea580c", fontSize: "0.8rem", textTransform: "uppercase", letterSpacing: "0.05em" }}>Missed Adjectives</p>
          {missed.map((m, i) => (
...
export default AdjectiveHuntResults;
