...
import PARAGRAPHS from "./adjectiveHuntData";
import AdjectiveHuntParagraph from "./AdjectiveHuntParagraph";
import AdjectiveHuntPopup from "./AdjectiveHuntPopup";
import AdjectiveHuntResults from "./AdjectiveHuntResults";

const AdjectiveHuntGame = () => {
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {popup && (
        <AdjectiveHuntPopup token={paragraph.tokens[popup.tokenIndex]} onAnswer={handleAnswer} onClose={() => setPopup(null)} position={popup.position} />
      )}
    </div>
  );
};

export default AdjectiveHuntGame;
