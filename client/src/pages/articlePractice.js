...
const ArticlePractice = () => {
  const sortingQuestions = articleData.filter((q) => q.practice_type === "sorting");
  const typingQuestions = articleData.filter((q) => q.practice_type === "typing");
  const mcQuestions = articleData.filter((q) => q.practice_type === "multiple_choice");

  const games = [
    { id: "mc", title: "📝 Article Identification Challenge", color: "green.500", component: <MultipleChoiceWrapper questionsToPlay={mcQuestions} nextPath="/article-structure" /> },
    { id: "typing", title: "✍️ Typing Challenge", color: "purple.500", component: <TypingGameWrapper questionsToPlay={typingQuestions} nextPath="/article-structure" /> },
    { id: "sorting", title: "🔀 Article Sorting", color: "#FF5722", component: <SortingGameWrapper questionsToPlay={sortingQuestions} nextPath="/article-structure" /> },
    { id: "hunt", title: "🔍 Article Hunt", color: "yellow.500", component: <ArticleHuntGame /> },
      ...
      <Box maxW="800px" mx="auto">
        {completed ? (
          <GameCard variant="game" textAlign="center" py={12}>
            ...
            <Heading size="lg" color="ink.700" mb={3}>You've completed all the Article games!</Heading>
            <Text fontSize="md" color="gray.600" mb={6}>Great work mastering articles through Multiple Choice, Typing, Sorting, Article Hunt, and AI Practice.</Text>
          ...
          <GameCard variant="game">
            ...
            {isAI ? (
              <GrammarAIPractice topic="articles" onComplete={handleNext} />
            ) : (
              <>
                {current.component}
