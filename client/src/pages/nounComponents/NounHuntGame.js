...
const NounHuntGame = () => {
  ...
  const totalItems = paragraph.tokens.filter((t) => t.isNoun).length;
  ...
  const handleAnswer = (choice) => {
    ...
    setPopup(null);
    const answered = Object.keys(newStates).filter((k) => paragraph.tokens[k]?.isNoun).length;
    if (answered === totalItems) {
      const missedList = paragraph.tokens.filter((t, i) => t.isNoun && newStates[i] !== "correct").map((t) => ({ word: t.word, type: t.type }));
  ...
  if (gameOver) {
    return (
      <div style={{ fontFamily: "'Inter', sans-serif" }}>
        <NounHuntResults score={score} total={totalItems} missed={missed} onReplay={handleReplay} />
      </div>
    );
  }
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: "16px", padding: "24px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
        ...
        <div style={{ background: "#ecfeff", border: "1px solid #a5f3fc", borderRadius: "10px", padding: "10px 14px", marginBottom: "20px" }}>
          <p style={{ margin: 0, fontSize: "0.85rem", fontWeight: "600", color: "#0e7490" }}>
            📖 Find every noun in the sentence, then identify its type!
          </p>
        </div>
        
        <NounHuntParagraph tokens={paragraph.tokens} tokenStates={tokenStates} onTokenClick={handleTokenClick} />
        {wrongClicks > 0 && (
      ...
      {popup && (
        <NounHuntPopup token={paragraph.tokens[popup.tokenIndex]} onAnswer={handleAnswer} onClose={() => setPopup(null)} position={popup.position} />
      )}
    </div>
  );
};
