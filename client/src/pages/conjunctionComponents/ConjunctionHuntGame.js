...
import PARAGRAPHS from "./conjunctionHuntData";
import ConjunctionHuntParagraph from "./ConjunctionHuntParagraph";
import ConjunctionHuntPopup from "./ConjunctionHuntPopup";
import ConjunctionHuntResults from "./ConjunctionHuntResults";

const ConjunctionHuntGame = () => {
  ...
  const totalItems = paragraph.tokens.filter((t) => t.isConj).length;
  ...
  const handleAnswer = (choice) => {
    ...
    setPopup(null);
    const answered = Object.keys(newStates).filter((k) => paragraph.tokens[k]?.isConj).length;
    if (answered === totalItems) {
      const missedList = paragraph.tokens.filter((t, i) => t.isConj && newStates[i] !== "correct").map((t) => ({ word: t.word, type: t.type }));
  ...
  if (gameOver) {
    return (
      <div style={{ fontFamily: "'Inter', sans-serif" }}>
        <ConjunctionHuntResults score={score} total={totalItems} missed={missed} onReplay={handleReplay} />
      </div>
    );
  }
    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      <div style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: "14px", padding: "16px 20px", marginBottom: "16px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
          <span style={{ fontWeight: "700", fontSize: "0.9rem", color: "#1f2937" }}>Conjunctions Found</span>
      ...
      <div style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: "16px", padding: "24px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
        ...
        <div style={{ background: "#ecfeff", border: "1px solid #a5f3fc", borderRadius: "10px", padding: "10px 14px", marginBottom: "20px" }}>
          <p style={{ margin: 0, fontSize: "0.85rem", fontWeight: "600", color: "#0e7490" }}>
            📖 Find every conjunction in the sentence, then identify its type!
          </p>
        </div>
        
        <ConjunctionHuntParagraph tokens={paragraph.tokens} tokenStates={tokenStates} onTokenClick={handleTokenClick} />
        {wrongClicks > 0 && (
      ...
      {popup && (
        <ConjunctionHuntPopup token={paragraph.tokens[popup.tokenIndex]} onAnswer={handleAnswer} onClose={() => setPopup(null)} position={popup.position} />
      )}
    </div>
  );
};

export default ConjunctionHuntGame;
