...
const VerbTenseQuizPage = () => {
  ...
  const generateQuiz = () => {
    // For verb questions, we need to create options from the answers
    const validQuestions = quizData.filter(q => q.answer && q.answer.length > 0).map(q => {
      ...
      // Otherwise, create options from answers and add some distractors
      const correctAnswers = q.answer;
      const distractors = ['run', 'ran', 'running', 'runs', 'walked', 'walks', 'walking', 'will walk',
                           'eat', 'ate', 'eating', 'eats', 'play', 'played', 'playing', 'plays',
                           'go', 'went', 'going', 'goes', 'have gone', 'had gone', 'will go'];

      // Create 4 options: use first correct answer and 3 distractors
      const options = [correctAnswers[0]];
      const filteredDistractors = distractors.filter(d => !correctAnswers.includes(d));
