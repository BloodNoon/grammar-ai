...
const VerbTenseStructure = () => {
  ...
  const analyzeVerbTenses = (words) => {
    ...
    const mainVerbs = words.filter(w => w.type === 'Verb' && w.subtype !== 'Auxiliary');

    if (mainVerbs.length === 0) {
      return { isValidCombination: false, feedback: 'No main verb found. Add an action word!' };
    }
    ...
    if (auxiliaries.length > 0 && mainVerbs.length > 0) {
      ...
      if ((aux === 'is' || aux === 'are' || aux === 'was' || aux === 'were') && mainVerb.subtype === 'Continuous') {
        return { isValidCombination: true, feedback: 'Perfect! This is a continuous tense.' };
      }
      if ((aux === 'have' || aux === 'has') && mainVerb.subtype === 'Perfect') {
        return { isValidCombination: true, feedback: 'Excellent! This is a perfect tense.' };
      }
      if (aux === 'will') {
        return { isValidCombination: true, feedback: 'Great! This is a future tense.' };
      }
    }

    if (mainVerbs.length > 0 && auxiliaries.length === 0) {
      return { isValidCombination: true, feedback: 'Good! Simple present or past tense.' };
    }

    return { isValidCombination: false, feedback: 'The verb combination doesn\'t match a standard tense pattern.' };
  };
  ...
  const generateVerbWordSet = () => {
    ...
    setAvailableWords([...new Set(words)].sort(() => Math.random() - 0.5).map((word, i) => ({
      id: `v-word-${i}`,
      text: word,
  ...
  // Drag and Drop Logic
  const handleDragStart = (e, word) => { setDraggedWord(word); };
  const handleDrop = (e) => {
        ...
        <Box display="grid" gridTemplateColumns={{ base: "1fr", lg: "1fr 1fr" }} gap={8}>
          ...
          <VStack align="stretch" spacing={6}>
            ...
            <Box bg="white" p={8} borderRadius="2xl" borderWidth="2px" borderColor="#1A1A1A" boxShadow="6px 6px 0px rgba(0,0,0,0.1)">
              <Heading size="lg" color="#1A0933" mb={4}>🎬 What is a Verb?</Heading>
              <Text fontSize="md" color="gray.600" mb={4}>
                Verbs are <strong>action</strong> words or <strong>states of being</strong>. Every sentence needs a verb!
              </Text>
              <Flex gap={4} wrap="wrap">
                <Box p={3} bg="blue.50" borderRadius="md" border="1px solid" borderColor="blue.300">
                  <Text fontWeight="bold" color="blue.700">Action</Text>
                  <Text fontSize="sm">run, eat</Text>
                </Box>
                <Box p={3} bg="green.50" borderRadius="md" border="1px solid" borderColor="green.300">
                  <Text fontWeight="bold" color="green.700">Linking</Text>
                  <Text fontSize="sm">is, am</Text>
                </Box>
                <Box p={3} bg="purple.50" borderRadius="md" border="1px solid" borderColor="purple.300">
                  <Text fontWeight="bold" color="purple.700">Helping</Text>
          ...
          <VStack align="stretch" spacing={6}>
            ...
            {/* Interactive Builder */}
            <Box bg="white" p={6} borderRadius="2xl" borderWidth="2px" borderColor="#1A1A1A" boxShadow="6px 6px 0px rgba(0,0,0,0.1)">
              <Heading size="md" color="#2196F3" mb={4}>🎮 Sentence Builder</Heading>

              <VerbProgressTracker correctCount={correctCount} totalAttempts={totalAttempts} streak={streak} TARGET_CORRECT={TARGET_CORRECT} />
              <VerbCompletionCelebration isCompleted={isCompleted} resetProgress={() => setCorrectCount(0)} />
              <VerbLevelSelection currentLevel={currentLevel} setCurrentLevel={setCurrentLevel} />

              <Text fontWeight="bold" my={4} color="blue.600">Drag words to build a perfect sentence:</Text>

              <VerbWordBank availableWords={availableWords} handleDragStart={handleDragStart} />
              <VerbSentenceBuilder sentenceArea={sentenceArea} handleDragOver={(e)=>e.preventDefault()} handleDrop={handleDrop} removeFromSentence={(i)=>removeFromSentence(i)} />

              <VerbActionButtons checkSentence={checkSentence} resetSentenceOnly={resetSentenceOnly} sentenceArea={sentenceArea} isCompleted={isCompleted} />
